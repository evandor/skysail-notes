...
//    def getId(): String = id
  ...
  def content_of_TestNote_can_be_deserialized_from_form() = {
    ...
    val testNote = FormDeserializer.createFrom(form, classOf[TestNote]).extract[TestNote]
    assertThat(testNote.content, is("theContent"))
  }

  @Test
  def content_of_Note_can_be_deserialized_from_form() = {
   // form.add("testNote|id", "42")
    form.add("testNote|content", "theContent")
    val testNote = FormDeserializer.createFrom(form, classOf[Note]).extract[Note]
    assertThat(testNote.content, is("theContent"))
  }
